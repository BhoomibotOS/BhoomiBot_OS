 ...
 * UPDATED: Added Over-The-Air (OTA) wireless flashing capabilities.
 */
...
#define BT_DEVICE_NAME "ESP32_Robot123"
...
// --- OTA Configuration ---
#define OTA_WIFI_SSID "BhoomiBot_Update"
#define OTA_WIFI_PASS "bhoomibot123"
...
unsigned long otaStartTime = 0; // To track how long we've been in OTA mode
#define OTA_TIMEOUT_MS 300000   // 5 minute safety timeout
int curDacL = 0, curDacR = 0;
String btBuf;
bool g_otaMode = false; // Flag to indicate if we are in maintenance/update mode
...
void loop() {
  if (g_otaMode) {
    // MAINTENANCE MODE: Only handle web requests for OTA
    server.handleClient();
    ElegantOTA.loop();

    // Visual feedback: Fast Triple-Pulse during OTA mode
    unsigned long m = millis() % 1000;
    bool ledOn = (m < 100) || (m > 200 && m < 300) || (m > 400 && m < 500);
    digitalWrite(STATUS_LED, ledOn ? HIGH : LOW);

    // Safety Timeout: Reboot if no update happens within 5 minutes
...
void handleCommand(String cmd) {
  ...
  // OTA TRIGGER: Enter wireless update mode
  if (cmd == "OTA") {
    enterOtaMode();
    return;
  }
  ...
  if (cmd.startsWith("SPD")) {
    ...
    switch (c) {
      ...
      case 'R':
        ...
        g_brakes = false;
        break;
      case 'S': g_turnIntensity = 0; g_dir = STOP; g_brakes = false; break;
...
/**
 * Safety Shutdown and WiFi Boot for Over-The-Air updates.
 */
void enterOtaMode() {
  Serial.println("Entering OTA Mode...");

  // 1. HARDWARE SAFETY: Stop all movement and lock brakes
  g_dir = STOP;
  g_speed = 0;
  curDacL = 0;
  curDacR = 0;
  ...
  g_brakes = true;
  digitalWrite(BRAKE_PIN_L, BRAKE_ON);
  digitalWrite(BRAKE_PIN_R, BRAKE_ON);

  // 2. ACKNOWLEDGE TO APP & CLEANUP RADIO
  SerialBT.println("OTA_STARTING");
  SerialBT.flush();
  delay(500); // Give time for BT buffer to clear
  SerialBT.end();

  // 3. START WIFI ACCESS POINT
  WiFi.mode(WIFI_AP);
  WiFi.softAP(OTA_WIFI_SSID, OTA_WIFI_PASS);
  Serial.print("WiFi AP Started: "); Serial.println(OTA_WIFI_SSID);
  Serial.print("IP Address: "); Serial.println(WiFi.softAPIP());

  // 4. START WEB SERVER & OTA
  ElegantOTA.begin(&server);
  server.begin();

  g_otaMode = true;
  otaStartTime = millis();
  Serial.println("OTA Server Ready at /update");
}
